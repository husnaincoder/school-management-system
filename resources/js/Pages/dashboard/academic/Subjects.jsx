...
const emptyForm = { name: '', code: '', type: 'Theory', is_active: true };

export default function SubjectsIndex({ subjects = [], filterIsActive = '' }) {
    ...
    const handleDelete = (item) => {
        ...
        router.delete(route('academic.subjects.destroy', item.id), { preserveScroll: true });
    };

    const statusFilter = filterIsActive === '' ? '' : (filterIsActive === true || filterIsActive === '1' ? '1' : '0');
    const applyStatusFilter = (value) => {
        router.get(route('academic.subjects'), value === '' ? {} : { is_active: value }, { preserveState: true });
    };
                    ...
                    <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-gray-500 mt-0.5">Manage school subjects (e.g. Mathematics, Physics, Chemistry)</p>
                            </div>
                        </div>
                        <Link
                    ...
                    <div className="bg-white rounded-xl border border-gray-200 shadow-[0_1px_3px_rgba(0,0,0,0.08)] overflow-hidden">
                        <div className="px-6 py-4 border-b border-gray-200 flex flex-wrap items-center justify-between gap-4">
                            <div>
                                ...
                                <p className="text-sm text-gray-500 mt-0.5">Total: {list.length} subject{list.length !== 1 ? 's' : ''}</p>
                            </div>
                            <button
                                type="button"
                        ...
                        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                            <div className="bg-white rounded-lg p-6 w-full max-w-md">
                                <div className="flex justify-between items-center mb-4">
                                    <h2 className="text-xl font-semibold">{editingItem ? 'Edit Subject' : 'Add Subject'}</h2>
