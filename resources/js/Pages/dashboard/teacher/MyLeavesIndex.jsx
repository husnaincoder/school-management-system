...
export default function MyLeavesIndex({ leaves, leaveTypes = [], filters = {} }) {
    ...
    const applyFilters = (e) => {
        e?.preventDefault();
        router.get(route('teacher.my-leaves.index'), { status: filterStatus }, { preserveState: true });
    };
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="mb-6">
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-4 mt-2">
                            ...
                            <Link
                                href={route('teacher.my-leaves.create')}
                                            ...
                                            <tr key={row.id} className="hover:bg-gray-50/80 transition-colors">
                                                ...
                                                <td className="px-6 py-4 text-sm">
                                                    <span className="flex flex-wrap gap-3">
                                                        <Link href={route('teacher.my-leaves.show', row.id)} className="text-[#FFA500] hover:text-[#e59400] font-medium inline-flex items-center gap-1">
                                                            <FontAwesomeIcon icon={faEye} className="text-xs" /> View
                                                        </Link>
                                                        {row.status === 'pending' && (
                                                            <Link href={route('teacher.my-leaves.edit', row.id)} className="text-gray-700 hover:text-gray-900 font-medium inline-flex items-center gap-1">
