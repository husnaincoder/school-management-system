...
export default function Marks({ examResults = [], subjectRecordsByExam = {} }) {
    const results = Array.isArray(examResults) ? examResults : [];
    const byExam = typeof subjectRecordsByExam === 'object' ? subjectRecordsByExam : {};

    return (
        <AuthenticatedLayout>
            <Head title="My Results & Marks" />
            <div className="py-8">
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    <div className="mb-6 flex items-center gap-4">
                        <Link href={route('dashboard.student')} className="text-gray-500 hover:text-gray-700">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h1 className="text-2xl font-bold text-gray-900">My Results & Marks</h1>
                                <p className="text-sm text-gray-500 mt-0.5">View your exam results and subject-wise marks</p>
                            </div>
                        </div>
                    </div>
                    ...
                    {results.length === 0 ? (
                        <div className="bg-white rounded-xl border border-gray-200 p-8 text-center text-gray-500">
                            ...
                            <p>No exam results yet. Results will appear here once published.</p>
                        </div>
                    ) : (
                                    ...
                                    <div key={r.id} className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                                        ...
                                        <div className="p-6">
                                            ...
                                            <div className="flex flex-wrap gap-2 mt-3">
                                                <a href={route('student.exam-result-sheet', exam.id)} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium bg-gray-100 text-gray-700 hover:bg-gray-200 border border-gray-200" download>
                                                    <FontAwesomeIcon icon={faFileCsv} /> Result Sheet (CSV)
                                                </a>
                                                <a href={route('student.exam-merit-list', exam.id)} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium bg-amber-50 text-amber-800 hover:bg-amber-100 border border-amber-200" download>
