...
export default function EmployeeEdit({ employee, departments = [], designations = [] }) {
    const deptList = Array.isArray(departments) ? departments : [];
    const desigList = Array.isArray(designations) ? designations : [];

    const form = useForm({
        name: employee?.user?.name ?? '',
        id_card_number: employee?.user?.id_card_number ?? '',
    ...
    const submit = (e) => {
        e.preventDefault();
        form.patch(route('hr.employees.update', employee.id), { preserveScroll: true });
    };
                    ...
                    <div className="mb-6 flex items-center justify-between gap-3">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-gray-500 mt-0.5">Role: <span className="font-medium text-amber-600">Employee</span></p>
                            </div>
                        </div>
                        <Link
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
                            ...
                            <div className="lg:col-span-7 bg-white rounded-xl border border-gray-200 shadow-[0_1px_3px_rgba(0,0,0,0.08)] p-6 h-full flex flex-col">
                                ...
                                <div>
                                    ...
                                    <input className={inputClass(form.errors.employee_id)} value={form.data.employee_id} onChange={(e) => form.setData('employee_id', e.target.value)} />
