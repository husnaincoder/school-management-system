...
const STATUS_BADGE = { present: 'bg-green-100 text-green-800', absent: 'bg-red-100 text-red-800', late: 'bg-amber-100 text-amber-800', leave: 'bg-gray-100 text-gray-800' };

export default function ChildAttendance({ student, enrollment, dailyRecords, monthly, yearly, percentage, academicSessions = [], filters = {} }) {
    const applyFilters = (e) => {
        e?.preventDefault();
        const form = e.target;
        router.get(route('parent.child.attendance', student.id), {
        ...
        return (
            <AuthenticatedLayout>
                <div className="py-8 max-w-4xl mx-auto px-4">
                    <h1 className="text-2xl font-bold text-gray-900">Attendance - {student?.user?.name ?? student?.full_name}</h1>
                    <p className="text-gray-500 mt-2">No enrollment found for this student.</p>
                    <Link href={route('dashboard.parent')} className="text-sky-600 hover:text-sky-800 text-sm font-medium mt-4 inline-block">Back to Dashboard</Link>
                </div>
            </AuthenticatedLayout>
        );
    }
    ...
    return (
        <AuthenticatedLayout>
            <Head title="Attendance - {student?.user?.name ?? student?.full_name}" />
                ...
                <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                    <table className="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody className="divide-y divide-gray-200 bg-white">
                            {items.length === 0 ? (
                                <tr><td colSpan={5} className="px-6 py-12 text-center text-gray-500">No attendance records in this range.</td></tr>
                            ) : items.map((att) => (
                                <tr key={att.id}>
                                    <td className="px-6 py-4 text-sm text-gray-900">{formatDate(att.attendance_session?.attendance_date)}</td>
                                    <td className="px-6 py-4"><span className={`inline-flex px-2.5 py-0.5 rounded text-xs font-medium ${STATUS_BADGE[att.status] ?? 'bg-gray-100 text-gray-800'}`}>{att.status}</span></td>
                    ...
                    {dailyRecords?.links && dailyRecords.links.length > 1 && (
                        <div className="px-6 py-4 border-t flex justify-end gap-1">
                            {dailyRecords.links.map((link, i) => (
                                <button key={i} type="button" onClick={() => link.url && router.get(link.url)} disabled={!link.url} className={`min-w-[2.25rem] px-2 py-1.5 rounded text-sm font-medium ${link.active ? 'bg-sky-500 text-white' : link.url ? 'bg-gray-200 hover:bg-gray-300' : 'bg-gray-100 cursor-not-allowed'}`} dangerouslySetInnerHTML={{ __html: link.label }} />
                            ))}
                        </div>
                    )}
                </div>
