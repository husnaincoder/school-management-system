...
export default function LeaveApplicationEdit({ application, child }) {
    ...
    const submit = (e) => {
        e.preventDefault();
        put(route('parent.leave-applications.update', application?.id));
    };

    const childLabel = child?.user?.name ?? child?.name ?? '—';
    const classLabel = child?.enrollment?.class_label ?? '—';
        ...
        <AuthenticatedLayout>
            ...
            <div className="py-8">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="mb-6">
                        <Link href={route('parent.leave-applications.show', application?.id)} className="text-gray-500 hover:text-gray-700 text-sm font-medium">← Back to application</Link>
                        <div className="flex items-center gap-3 mt-2">
                            <div className="w-12 h-12 rounded-lg flex items-center justify-center bg-[#e8eef7]">
                                <FontAwesomeIcon icon={faCalendarXmark} className="text-amber-500 text-xl" />
                            </div>
                            <div>
                                <h1 className="text-2xl font-bold text-gray-900">Edit Leave Application</h1>
                                <p className="text-sm text-gray-500 mt-0.5">{childLabel} – {classLabel}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-xl border border-gray-200 shadow-[0_1px_3px_rgba(0,0,0,0.08)] p-6">
                        <form onSubmit={submit} className="space-y-5">
                            ...
                            <div className="flex flex-wrap gap-3 pt-2">
                                ...
                                <Link
                                    href={route('parent.leave-applications.show', application?.id)}
