...
import { faChalkboardTeacher, faArrowLeft, faEye, faUpload } from '@fortawesome/free-solid-svg-icons';
...
export default function TeacherEdit({ teacher }) {
    const quals = (teacher.qualifications && teacher.qualifications.length) ? teacher.qualifications.map((q) => ({
        degree_name: q.degree_name ?? '',
        field_of_study: q.field_of_study ?? '',
        board_university: q.board_university ?? '',
        passing_year: q.passing_year ?? '',
        grade_division: q.grade_division ?? '',
        certificate_image: q.certificate_image ?? null,
    })) : [{ ...emptyQualification }];

    const form = useForm({
        name: teacher.user?.name ?? '',
        id_card_number: teacher.user?.id_card_number ?? '',
        ...
        cv: teacher.cv ?? null,
        id_card_front: teacher.id_card_front ?? null,
        id_card_back: teacher.id_card_back ?? null,
    ...
    const submit = (e) => {
        e.preventDefault();
        form.post(route('admin.teachers.update', teacher.id), {
            preserveScroll: true,
            forceFormData: true,
            _method: 'patch',
        });
    ...
    const FileDrop = ({ fieldKey, label, accept, file, onFile, onClear, inputRef, compact }) => {
        ...
        const currentName = file && typeof file === 'string' ? file : null;
            ...
            <div>
                ...
                {isNewFile ? (
                    <div className="flex items-center justify-between gap-3 p-3 rounded-lg border border-gray-200 bg-gray-50">
                        <span className="text-sm font-medium text-green-700 truncate flex-1" title={file.name}>New: {file.name}</span>
                    ...
                    <>
                        {currentName && <p className="text-gray-600 text-sm mb-2">Current: {currentName}</p>}
                        <div
                        ...
                        >
                            <div className="flex flex-col items-center gap-1.5 pointer-events-none">
                                ...
                                <span className="text-sm font-medium text-gray-600">{isDragging ? 'Drop file here' : currentName ? 'Drag & drop or click to replace' : 'Drag & drop or click'}</span>
                            </div>
                        </div>
                    </>
                )}
                ...
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    ...
                    <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-gray-500 mt-0.5">Role: <span className="font-medium text-amber-600">Teacher</span></p>
                            </div>
                        </div>
                        <div className="flex items-center gap-2">
                            <Link href={route('admin.teachers.show', teacher.id)} className="inline-flex items-center gap-2 bg-gray-500 hover:bg-gray-600 text-white font-medium py-2 px-4 rounded-lg">
                    ...
                    <form onSubmit={submit} className="space-y-6">
                        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
                            <div className="lg:col-span-5 flex flex-col gap-6">
                                <div className="bg-white rounded-xl border border-gray-200 shadow-[0_1px_3px_rgba(0,0,0,0.08)] p-6">
                                    ...
                                    <p className="text-sm text-gray-500 mb-4">Role: <span className="font-medium text-amber-600">Teacher</span></p>
                            ...
                            <div><label className="block text-sm font-medium text-gray-700 mb-2">Experience</label><textarea className={inputClass()} rows={2} value={form.data.experience} onChange={(e) => form.setData('experience', e.target.value)} /></div>
                            <div><label className="block text-sm font-medium text-gray-700 mb-2">Address</label><textarea className={inputClass()} rows={2} value={form.data.address} onChange={(e) => form.setData('address', e.target.value)} /></div>
                        ...
                        <div className="flex justify-end gap-3">
                            ...
                            <button type="submit" disabled={form.processing} className="bg-amber-500 hover:bg-amber-600 text-white font-medium py-2 px-6 rounded-lg disabled:opacity-50">{form.processing ? 'Updating...' : 'Update'}</button>
                        </div>
                    </form>
                </div>
            </div>
