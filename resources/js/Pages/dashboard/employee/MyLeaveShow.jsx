...
export default function MyLeaveShow({ leave }) {
    ...
    const handleDelete = () => {
        if (confirm('Delete this leave application? This cannot be undone.')) {
            router.delete(route('employee.my-leaves.destroy', leave?.id), {
                onSuccess: () => router.get(route('employee.my-leaves.index')),
            });
        }
    };
        ...
        <AuthenticatedLayout>
            ...
            <div className="py-8">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="mb-6">
                        <Link href={route('employee.my-leaves.index')} className="text-gray-500 hover:text-gray-700 text-sm font-medium">← Back to list</Link>
                        <div className="flex flex-wrap items-center justify-between gap-4 mt-2">
                            ...
                            {leave?.status === 'pending' && (
                                <span className="flex gap-2">
                                    <Link
                                        href={route('employee.my-leaves.edit', leave.id)}
